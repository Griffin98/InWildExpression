...
from networks.encoder_inject_concat import StylizedExpressionEncoder
...
class ExpressionModule(pl.LightningModule):
    ...
    def setup_deca(self):
        ...
        self.exp_loss = ExpLoss(model=self.deca_model)
    ...
    def g_non_exp_saturating_loss(self, fake_pred, fake_img, real_img, target_expression):
        ...
        if self.opts.lambda_l1 > 0:
            ...
            dict['l1_loss'] = loss_l1
            loss += self.opts.lambda_l1 * loss_l1

        if self.opts.lambda_lpips > 0:
            loss_lpips = self.lpips_loss(fake_img, real_img)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if self.opts.training_stage == "imitate":
            ...
            expression = torch.randn(batch.shape[0], self.opts.expression_dim)
            expression = torch.clamp(expression, min=-2, max=2)
    ...
    def log_image(self, batch, expression):
        ...
        if self.opts.training_stage == "contrastive":
            n_row = 3
...
class ExpressionModel(nn.Module):
    def __init__(self, opts):
        ...
        if opts.load_stylegan_weights and opts.training_stage == "imitate":
            print("Loading stylegan weights")
            pretrained_dict = torch.load(self.opts.stylegan_weights)["g_ema"]
            modified_dict = load_indexed_stylegan_dict(self.decoder, pretrained_dict)
            self.decoder.load_state_dict(modified_dict)

    def forward(self, image, expression, return_latents=True):
        embedding, noise = self.encoder(image)
        #
        # embedding[:, 412:55] = torch.randn(100)

        style = torch.cat((embedding, expression), dim=1)

        style[:, 250:275] = torch.randn(25)

        y, latent = self.decoder([style], noise=noise, return_latents=return_latents)
...
if __name__ == "__main__":
    ...
    exp = torch.randn(4, 50).to("cuda")

    out, _ = expModel(img, exp)

    from torchvision.utils import save_image
    save_image(out, "grid.png")
    print(out.shape)